...
export function BEencrypt(text) {
    const cipher = crypto.createCipheriv(algorithm, Buffer.from(backend_secret, "utf-8"), iv);
    let encrypted = cipher.update(text, "utf-8", "hex");
    encrypted += cipher.final("hex");
    return `${iv.toString("hex")}:${encrypted}`;
  }
...
export function decrypt(encryptedData) {
  ...
  const decipher = crypto.createDecipheriv(algorithm, Buffer.from(secretKey, "utf-8"), Buffer.from(ivHex, "hex"));
...
export function BEdecrypt(encryptedData) {
    const [ivHex, encryptedText] = encryptedData.split(":");
    const decipher = crypto.createDecipheriv(algorithm, Buffer.from(backend_secret, "utf-8"), Buffer.from(ivHex, "hex"));
    let decrypted = decipher.update(encryptedText, "hex", "utf-8");
    decrypted += decipher.final("utf-8");
    return decrypted;
  }
